...
class InvalidResponseTransport:
    def post_json(self, url, payload, headers, timeout):
        # Missing 'score' field completely
        return {"model_mode": "trained", "hard_constraint_valid": True}
...
def test_ml_invalid_response_triggers_fallback() -> None:
    settings = load_settings({"ML_MODELS_INTERNAL_URL": "http://fake-ml:8000"})
    client = MLModelsClient(settings, transport=InvalidResponseTransport())
    ...
    assert optimization_trace["detail"] == "dependency_contract_error"
